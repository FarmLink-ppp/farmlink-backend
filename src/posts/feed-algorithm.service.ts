...
export class FeedAlgorithmService {
  ...
  calculatePostScore(
    ...
    const likeScore =
      this.normalizeCount(post.likes.length) * this.weights.likeWeight;
    const commentScore =
      this.normalizeCount(post.comments.length) * this.weights.commentWeight;
    const shareScore =
      this.normalizeCount(post.shares.length) * this.weights.shareWeight;
  ...
  getRankedFeed(
    ...
    const scoredPosts = posts.map((post) => ({
      post,
      score: this.calculatePostScore(post, followingMap[post.user_id] || false),
    }));
